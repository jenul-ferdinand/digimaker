...
const ChallengeSchema = z.object({
  name: z.string().describe('The name of the specific challenge'),
  task: z.string().describe('The task for the challenge, explained as a requirement or feature to add'),
})
...
const ParsedLessonSchema = z.object({
  ...
  projectImage: z.string().nullable().default(null).describe('Reference to the main project image if present'),
  ...
  codeBlock: z.string().nullable().default(null).describe('A single code block if the lesson uses text-based code instead of visual blocks'),
  tryItOutSection: z.array(z.string()).nullable().default(null).describe('Steps to test the project after coding'),
  challengeSection: z.array(ChallengeSchema).describe('Challenge tasks for students to extend the project'),
  ...
  funFact: z.string().nullable().default(null).describe('An interesting fact related to the lesson topic'),
});
